...
class PickCubeStateDataset(Dataset):
    """State/action chunks from replayed ManiSkill PickCube demonstrations."""

    def __init__(
        self,
        traj_path: str = "~/.maniskill/demos/PickCube-v1/rl/trajectory.state.pd_ee_delta_pos.physx_cpu.h5",
        ...
        if not self.traj_path.exists():
            raise FileNotFoundError(f"PickCube trajectory file not found: {self.traj_path}")
    ...
    def _build_indices(self) -> list[tuple[int, int]]:
        indices = []
        min_tail = max(self.action_horizon, self.future_horizon)
        for traj_idx, traj in enumerate(self.trajectories):
            action_len = traj["actions"].shape[0]
            for step in range(max(action_len - min_tail + 1, 0)):
                indices.append((traj_idx, step))
        if not indices:
            raise RuntimeError("No valid chunk indices. Reduce action_horizon/future_horizon.")
        return indices
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        obs = torch.from_numpy(traj["obs"][step]).float()
        action = torch.from_numpy(traj["actions"][step : step + self.action_horizon]).float()
        future_obs = torch.from_numpy(traj["obs"][step + 1 : step + 1 + self.future_horizon]).float()
        if self.normalize:
            obs = (obs - self.stats["obs_mean"]) / self.stats["obs_std"]
            future_obs = (future_obs - self.stats["obs_mean"]) / self.stats["obs_std"]
